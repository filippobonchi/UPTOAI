...
\institute{University of Pisa, Italy \and University of Verona \& IMDEA SW Institute  \and University of Hawaii in Manoa}
...
%Observe that if it exists the adjoint is unique. We will often use the fact that a map $f$ is a left adjoint iff it preserves $\bigsqcup$ (in particular $\bot$) and a right adjoint iff it preserves $\bigsqcup$ (in particular $\top$).
...
\marginpar{Check whether in the original work of Milner, he considers just up-closures}Observe that, according to the above definition an up-to technique $a$ might not be \emph{complete}: it may exist an $i$ such that $i \sqsubseteq \nu b$ for which there is no $x$ satisfying  $i \sqsubseteq x\sqsubseteq ba(x)$. However, if $a$ is an up-closure operator, then $\nu b  \sqsubseteq a(\nu b)$ and using monotonicity of $b$, one obtains that 
...
\marginpar{To decide which formulation we want to give}
\begin{lemma}
...
%If $\mu b \sqsubseteq_C f$, then $\mu \overline{b} = \alpha(\mu b)\sqsubseteq_A \alpha(f) = f$.
\end{enumerate}
\end{proof}

%\marginpar{The proof is partially in the Appendix and partially in a mail by Roberto}
...
%%
%% OLD VERSION OF THIS SUBSECTION. REVISITED MANY TIMES: One can copy old proofs if needed
%%
%%\subsection{Abstract domains}
...
%
%%
%This is more evident with the following alternative characterization.
%
%\begin{lemma}\label{lemma:alternativecompleteness}
%$a$ is $b$-sound iff $a(\mu b) \sqsubseteq_C \mu a b $.
%$a$ is $b$-complete iff $a(\mu b) = \mu (a b) $.
%\end{lemma}
%\begin{proof}
%%
%This is Lemma 3.1 of \cite{DBLP:journals/jacm/GiacobazziRS00}. For reader convenience, we report its proof.
%%
...
%\medskip
%
%We can now prove the right-to-left implication. It always hold that $\alpha(\mu b) \sqsubseteq \mu (\alpha b \gamma)$ (this is just soundness of $\overline{b}^a$). For the other inclusion, observe that by assumption we have $a(\mu b) = \mu (a b) $ which entails that $\gamma \alpha b \gamma \alpha (\mu b)= \gamma \alpha (\mu b)$. Since $\gamma$ is injective, $ \alpha b \gamma \alpha (\mu b)=  \alpha (\mu b)$, that is $\alpha (\mu b)$ is a fixed point of $\alpha b \gamma $. Thus $\mu(\alpha b \gamma) \sqsubseteq \alpha (\mu b)$.
%\end{proof}
%
%%
%%\begin{lemma}
%%Let $f\in Pre(a)=A$. Then
%%$$\mu \overline{b}^a \sqsubseteq_A f \quad \text{ iff } \quad  \mu (ab)\sqsubseteq_C f$$
%%\end{lemma}
%%\begin{proof}
%%$\mu \overline{b}^a \sqsubseteq_A f$ iff $\gamma(\mu \overline{b}^a) \sqsubseteq_C \gamma(f)=f$ iff, by Lemma \ref{}, $a(\mu(ab))\sqsubseteq_C f$. If  $a(\mu(ab))\sqsubseteq_C f$, then $\mu (ab) \sqsubseteq_C a(\mu(ab))\sqsubseteq_C f$. If $\mu (ab)\sqsubseteq f$ then $a(\mu(ab))\sqsubseteq a(f)=f$.
%%\end{proof}
%
%
%%In Section \ref{}, we will see that the problems of proving soundness of up-to techniques and completeness of abstract domains are intimately related. We first show an example of abstract interpretation at work.
%
...
\marginpar{This part could be rephrased: we could decide to call soundness just the one at the fix point, or avoid completely the definition}
...
\end{proposition}
\begin{proof}
\marginpar{Take care of the proof somehow}
...
\begin{remark}
\marginpar{Please Roberto, take care of this remark}
...
%
\marginpar{Please Roberto, insert citations here.}A number of works  \cite{} has studied modularity for proofs of full-completeness of abstract domains, but always focusing on up-closures rather than on arbitrary maps. This example, together with the results in Section \ref{}, shows that also for abstract domains could be convenient to decompose up-closures into smaller monotone maps.
\end{remark}
...
%
%%
%Following the example in Sections \ref{sec:HKsoundness} and \ref{sec:Signcomplete}, we restrict our attention to those monotone maps $b\colon C \to C$ that can be decomposed as either $b=i \sqcup b^*$ or $b= b_* \sqcap f$.  %for some $i,f\in C$ and a pair of adjoints $b^*\dashv b_*\colon C \to C$. Intuitively, the elements of $C$ represent some predicates, or conditions, $i$ and $f$ intial and final condifitions, and $b^*$ and $b_*$ predicate transformers mapping a condition into its strongest postcondition and weakest precondition.
%%
...
%
%
\marginpar{TO decide whether to keep it as a theorem or not...}
\begin{theorem}
...
\subsection{Hopcroft and Karp as complete abstract interpretation}\marginpar{Skip all this subsection, should be rewritten later}
...
\marginpar{To rewrite everything here. First we should read the original paper by Hopcroft and Karp}
...
where $\oplus \!1 \colon Pred_Z \to Pred_Z$ is defined as $P\oplus \!1 = \{i+1\mid i\in P  \}$. %, for all $P\in \Pred_Z$.
...
$$ 
\marginpar{Please check this computation. Maybe in the submitted version, we could remove it.}
...
\section{Intermezzo} \marginpar{All This section is just a rough Skeleton that I would like to have in the middle of the paper. But this is not strictly necessary. PLEASE SKIP IT FOR THE MOMENT}
%
...
The answer is no.  Consider the following lattice, with $b^*$ defined by the dashed lines on the left and $b_*$ defined by the dotted lines on the right. It is easy to see that they are adjoints. \marginpar{Please check that they are adjoint carefully!!!}Take $i=1$ and $f=4$. Let $a$ be the up-closure operator, such that $Pre(a)= \{\}$
...
\marginpar{to be moved somewhere else}
...
This allows us to give a characterisation of $\omega$ as an abstract domain $\Omega$ with Galois insertion $\langle \alpha,\gamma\rangle \colon C \to \Omega$ which will be convenient later for abstract interpretation. %To the best of our knowledge, such characterisation is novel: this can be easily derived from exisiting results, but we found it useful and, somehow, more evocative than those that we have seen so far.

\marginpar{TO BE MOVED IN THE APPENDIX, AFTER WE PROVED IT}
\begin{lemma}\label{lemma:inclusion}
$a_1\sqsubseteq a_2$ iff $Pre(a_2) \subseteq Pre(a_1)$
\end{lemma}
\begin{proof}\marginpar{finish this proof}
...
%\marginpar{After the Corollary says something about the fact that this is not really useful for proving compatibility in general, since one has to know $\Omega$}



%\begin{lemma}\label{lemma:restrictcorestrict}
%Let $b,a \colon C \to C$ be a map and a closure operator. Then $a$ is $b$-compatible iff for all $x\in Pre(a)$, $b(x)\in Pre(a)$.
%\end{lemma}
%\begin{proof}
%The rightmost condition is just a reformulation of condition 3 in Lemma \ref{lemma:equivalentformulation}. Indeed $A= Pre(a)$, and $\gamma \colon Pre(a)\to C$ is just the obvious injection. Therefore $\overline{b}\colon Pre(a) \to Pre(a)$ is defined exactly as $b$ but on restricted domain and codomain. 
%\end{proof}



\marginpar{Please Roberto write this remark}
\begin{remark}
...
%
%
%\begin{proposition}\label{prop:fixpoint-bf}
...
%
%\begin{lemma} \marginpar{This lemma is not really needed}Let $C$ be a complete lattice, $b,a_1,a_2\colon C \to C$ be a monotone map and two up-closures such that $a_1\sqsubseteq a_2$. Let $f\in C$ such that $a_2(f)\sqsubseteq f$. Then $a_1(f)\sqsubseteq f$ and
...
\begin{theorem}[Fix-point completeness vs soundness]
