...
Another interesting fact about $b,f$-completenessis that it enjoys the following intuitive characterisation w.r.t fixed point completeness.
\begin{proposition}
Let $C$ be a complete lattice, $b,a\colon C \to C$ be a monotone map and a up-closure.
Then:
\begin{center}
$a$ is fix-point complete w.r.t. $b$ \\ iff \\ for all $f$ such that $a(f)\sqsubseteq f$, $a$ is $(b,f)$-complete. 
\end{center}
\end{proposition}
\begin{proof}
The top-down implication is just Proposition \ref{prop:fixpoint-bf}. For bottom up, one can take $f=a(\mu b)$ since $aa(\mu b)=a(\mu b)$. It holds $\mu b \sqsubseteq a(\mu b)$ and thus $\mu(ab) \sqsubseteq a(\mu b)$. The other inclusion $a(\mu b) \sqsubseteq \mu(ab)$ always holds (this is just soundness).
\end{proof}
